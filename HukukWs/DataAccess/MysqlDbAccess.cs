    ...
    public class MysqlDbAccess : IDisposable
    {
        ...
        public string ConnectionString
        {
            get
            {
                lock (_lock)
                {
                    return @"server=localhost;userid=hikmetyarbasi;password=Computer67.;database=hs_demo";
                }
            }
        }
        ...
        public List<T> QueryList<T>(string procName, List<MySqlParameter> parameter) where T : new()
        {
            lock (_lock)
            {
